...
var rows = 10, columns = 10, mines = 10, cellsHolder, queue = {}, gameIsRunning = true, gameIsStarted = false, remainingMines = 10;
(function () {
    ...
    function __openCell(curRow, curColumn) {
        ...
        __getCell(curRow,curColumn).removeClass('closed');
		__getCell(curRow,curColumn).addClass('zero');
        for (var i = Math.max(curRow - 1, 0); i <= Math.min(curRow + 1, rows); i++) {
            for (var j = Math.max(curColumn - 1, 0); j <= Math.min(curColumn + 1, columns); j++) {
